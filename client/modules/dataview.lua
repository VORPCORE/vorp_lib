...
for label, datatype in pairs(DataView.Types) do
    ...
    if datatype.size >= 0 then
        local packSize = string.packsize(datatype.code)
        if packSize ~= datatype.size then
            error(string.format("Pack size of %s (%d) does not match cached length: (%d)", label, packSize, datatype.size))
        end
    end
end
